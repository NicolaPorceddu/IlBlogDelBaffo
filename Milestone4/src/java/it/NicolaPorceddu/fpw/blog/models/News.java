...
public class News{
    ...
    public void setCategorie(int[] categorie){
        if(categorie[0] == 1)
            this.categorie.add("Politica");
        if(categorie[1] == 1)
            this.categorie.add("Cronaca");
        if(categorie[2] == 1)
            this.categorie.add("Esteri");
        if(categorie[3] == 1)
            this.categorie.add("Economia");
        if(categorie[4] == 1)
            this.categorie.add("Sport");
        if(categorie[5] == 1)
            this.categorie.add("Cultura");
    }
    ...
    /*Il metodo restituisce un Array di stringhe contenente le categorie degli articoli. Viene passato come 
      parametro una stringa, contenente i caratteri inseriti dall'utente nella barra di ricerca. Vengono
      restituite le categorie che contengono quella sequenza di caratteri.*/
    public static ArrayList<String> searchCategory(String toSearch){
        ArrayList<String> listCategory = new ArrayList<>();
        ArrayList<String> listToReturn = new ArrayList<>(); 
        
        listCategory.add("Politica");
        listCategory.add("Cronaca");
        listCategory.add("Esteri");
        listCategory.add("Economia");
        listCategory.add("Sport");
        listCategory.add("Cultura");
        
        for(String string : listCategory)
            if(string.toLowerCase().contains(toSearch.toLowerCase()))
                listToReturn.add(string);
        
        return listToReturn;
    }
    
    /*Se la barra di ricerca è vuota, vengono restituite tutte le categorie.*/
