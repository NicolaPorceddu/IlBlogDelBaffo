...
<html lang="it">
    ...
    <body>
        ...
        <main id="inputPage">
            ...
            <!--Se nel form non viene inserita almeno una categoria, il form viene ricaricato, senza salvare la notizia.-->
            <c:if test="${categorieError==true}">
                <h3>L'articolo deve contenere almeno una categoria!</h3>
            </c:if>
            
            <!--Nel caso si cerchi di modificare un articolo di un altro autore, si visualizzerà un messaggio di
                errore.-->
            <c:if test="${invalidAccess==true}">
                <br/><br/>
                <h3>Non hai i permessi per modificare questo articolo!</h3>
            </c:if>
            
            <c:if test="${invalidAccess!=true}">
                <h2 class="corsivo">Scrivi articolo</h2>
                <c:if test="${idNews!=null}">
                    <form action="scriviArticolo.html?save=1&nid=${idNews}" method="post">
                </c:if>
                <c:if test="${idNews==null}">
                    <form action="scriviArticolo.html?save=1" method="post">
                </c:if>

                    <label for="titolo" class="primo">Titolo</label>
                    <input type="text" name="titolo" id="titolo" class="grey" value="${news.titolo}" required/>

                    <!--Modificando un articolo, la data di pubblicazione non può essere modificata-->
                    <c:if test="${addDate==true}">
                        <label for="data" class="successivi">Data</label>
                        <input type="date" name="data" id="data" class="grey" value="${data}" required>
                    </c:if>

                    <label for="immagine" class="successivi">Immagine</label>
                    <input type="url" name="immagine" id="immagine" class="grey" value="http://${news.img}" required/>

                    <label for="didascalia" class="successivi">Didascalia</label>
                    <input type="text" name="didascalia" id="didascalia" class="grey" value="${news.didascalia}" required/>

                    <label for="contenuto" class="successivi">Testo dell'articolo</label>
                    <textarea rows="20" cols="80" name="contenuto" id="articolo" class="grey" required>${articolo}</textarea>
                    <br/><br/>
